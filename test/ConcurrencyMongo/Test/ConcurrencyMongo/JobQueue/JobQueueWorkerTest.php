...
class JobQueueWorkerTest extends PHPUnit_Framework_TestCase
{
        ...
        $worker->addJobWorker(
            ...
            new TestJobWorker(
                function ($job) use ($self, &$cnt) {
                    ...
                    if ($cnt==2) {
                        $self->assertEquals('value 001_02', $job->getValue());
                    }
                    if ($cnt==3) {
                        $self->assertEquals('value 001_03', $job->getValue());
                    }
                    $job->done();
                }
            )
    ...
    public function testJobExpired()
    {
        ...
        $worker->addJobWorker(
            'LABEL_001',
            new TestJobWorker(
                function ($job) use ($self, &$cnt, &$exceptions) {
                    ++$cnt;
                    $self->log->trace(sprintf('cnt:%d value:%s', $cnt, $job->getValue()));
